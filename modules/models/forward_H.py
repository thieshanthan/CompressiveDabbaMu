...
from ..custom_activations import sigmoid_custom
from .initialize_H_weights import *


class modelH_class(nn.Module):
    def __init__(self, T, img_size, preprocess_H_weights=None, device='cpu',
                 initialization_bias=0, activation=None, init_method='randn',
                 enable_train=True, lambda_scale_factor=1):
        super(modelH_class, self).__init__()
        if init_method == 'randn':
            init_weights = torch.randn((1, T, img_size, img_size),
                                       dtype=torch.float32)
            ...
            init_weights = torch.ones((1, T, img_size, img_size),
                                      dtype=torch.float32)
            init_weights = torch.fft.fft2(init_weights)

        elif init_method == 'canonical_FourierBased':
            init_weights = canonical(T, img_size,
                                     lambda_scale_factor)  # returns {0, 1} torch tensor with shape: (1, T, img_size, img_size)
            init_weights = torch.fft.fft2(init_weights)

        elif init_method == 'hadamard_FourierBased':
            init_weights = hadamard_norescale(T, img_size,
                                              lambda_scale_factor)  # returns {0, 1} torch tensor with shape: (1, T, img_size, img_size)
            init_weights = torch.fft.fft2(init_weights)
    ...
    def forward(self, m=1):
        ...
        if self.activation != None:
            x = self.activation(x, m)
        else:
            x = sigmoid_custom(x, m)
