...
class modelA_class:
    def __init__(self, sPSF, exPSF, noise=False, device='cpu', scale_factor=1,
                 rotation_lambda=1, shift_lambda_real=0, readnoise_std=0,
                 ways:int = 5, shots: int = 5):
        self.sPSF = sPSF
        self.exPSF = exPSF
        self.device = device
        ...
        self.ways = ways
    def forward_model_singleH(self, X,
                              Ht):  # X: (m, 1, Nx, Ny), Ht: (1, 1, Nx, Ny), sPSF: (N1, N1), exPSF: (N2, N2)
        sPSF = self.sPSF
        exPSF = self.exPSF
        ...
        sPSF_temp = torch.zeros((1, self.ways, sPSF.shape[0],
                                  sPSF.shape[1])).to(self.device)
    ...
    def compute_yt(self, X, H):
        lambda_up = torch.zeros(
            (X.shape[0], H.shape[1], X.shape[2], X.shape[3])).to(self.device)
        for t in range(H.shape[1]):
            lambda_up[:, t:t + 1, :, :] = self.forward_model_singleH(X, H[:,
                                                                        t:t + 1,
                                                                        :, :])

        lambda_down = lambda_up
        for _ in range(self.scale_factor - 1):  # downscaling
            lambda_down = F.avg_pool2d(lambda_down, kernel_size=2, stride=2,
                                       padding=0) * 4
        ...
        if self.noise == True:  # add noise
            ...
            yt_down = lambda_down + (
                        self.shift_lambda_real / self.rotation_lambda) + torch.sqrt(
                lambda_down / self.rotation_lambda + self.shift_lambda_real / (
                            self.rotation_lambda ** 2)) * z + read_noise / self.rotation_lambda
        else:
            yt_down = lambda_down
