...
def save_grids(data, images_per_grid, nrow_per_grid, start_idx, save_dir, type_,
               remove_existing_dir=False):
    if remove_existing_dir:
        try:
            shutil.rmtree(f"{save_dir}/{type_}")
        except:
            pass
    ...
    print(
        f'NO DUPLICATES in the dataset ...  --> uniques.shape ({uniques.shape}) == data.shape ({data.shape})')
    ## CHECK DUPLICATES

    for i in range(0, len(data), images_per_grid):
        grid = torchvision.utils.make_grid(data[i:i + images_per_grid],
                                           padding=0,
                                           nrow=nrow_per_grid).permute(1, 2,
                                                                       0).cpu().numpy()
...
try:
    ...
    print(f'available directory :: {save_dir}')
...
for idx in range(repeat_train):
    ...
    save_grids(train_data, images_per_grid, nrow_per_grid,
               idx * (len(train_data) // images_per_grid), save_dir, 'train',
               remove_existing_dir=remove_existing_dir)  # save 600 images
...
for idx in range(repeat_valtest):  # range(20*4):
    ...
    val_test_data = val_test_data[
        np.random.choice(np.arange(len(val_test_data)), len(val_test_data),
                         replace=False)]  ## SHUFFLE WITHOUT REPEATS!!!
    save_grids(val_test_data[:100], 1, 1,
               idx * (100 // images_per_grid), save_dir, 'val',
               remove_existing_dir=remove_existing_dir)  # save 50 images
    # save_grids(val_test_data[5000:], images_per_grid, nrow_per_grid,
    #            idx * (5000 // images_per_grid), save_dir, 'test',
    #            remove_existing_dir=remove_existing_dir)  # save 50 images
