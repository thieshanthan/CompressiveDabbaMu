...
def safe_do_exps(exps_dict= None, general_opts= None, device= None, exp_dir = '../figs/test', save_special= False, count_only= True, save_dir_special_root=None, run=None):    
    exp_idx = 0
    keys= list(exps.keys())
...
parser.add_argument('--exp_type', type=str, help='exp type: contentaware or segmentation', default= 'contentaware')
parser.add_argument('--dataset_name', type=str, help='dataset_name', default= 'mnistdigits_grid2patch')
parser.add_argument('--data_dir', type=str, help='dataset_name', default= './datasets/patchmnist')
...
exps = {
        ...
        'MODEL.MODEL_A.readnoise_std': ['0.0'], # readnoise standard deviation (e.g.: '6.0')
...
safe_do_exps(exps, general_opts, device, exp_dir = exp_dir, save_special= False, count_only= count_only, save_dir_special_root= save_dir_special_root, run= run)